...
**B3｜闭环性能未达标（16.9–18.6s vs 目标 ≤15s） 【已完成 · 根因经实测修正 · 高】**
- ⚠️ **实施期修正：本方案原先的根因判断是错的**。原判断是"日报与洞察两次串行真实 LLM 生成"；真机逐阶段拆解（SSE 事件里的 `duration_ms`）显示 `collection 6ms + governance ~1.0–1.5s + operation 6.6s ≈ 8.2s`，而**总耗时却是 16.6–18.1s** —— 有约 **10s 与三个阶段无关**。
- **真实根因（按贡献排序）**：
  1. **SSE 生成器空等心跳超时（约 10s，主因）**：执行任务结束、事件已全部推完后，生成器仍阻塞在 `asyncio.wait_for(queue.get(), timeout=HEARTBEAT_SECONDS=10)`，必须等满 10 秒才会重新检查 `task.done()`。这不是"LLM 慢"，而是端点自身的收尾缺陷（属 v2.1 我引入的）。
  2. **日报与洞察串行**（次因，体现在 operation 阶段内部）：两者无数据依赖却顺序执行。
- **已实施修复**：
  1. 执行任务在 `finally` 中向事件队列投递**结束哨兵** `(None, None)`，生成器收到即 break —— 收尾从"最多等 10s"变为立即；哨兵不对外推送，事件协议与回放同构性不变。
  2. 日报与洞察改用 `asyncio.create_task` + `asyncio.gather` **并行**，各自使用**独立 DB 会话**（并发写库不能共用同一个 AsyncSession）；单侧失败降级为 `partial` 不影响另一侧；两条决策仍各带 `finished_at`/`duration_ms`。
- **真机复测（真模型、不含采集、2 次）**：**8.7s / 7.8s**（P50 ≈ 8.2s），由 16.6–18.1s 降至 8s 级，**满足 ≤15s**；剩余主要是洞察的 LLM 生成（约 5.4s，外部延迟）。
- **附带收益**：后端闭环测试文件耗时 84s → 11s（原先每个用例都要等满一次心跳超时）。
- **验证（已执行）**：`pytest tests/test_orchestrator_parallel.py`（5 例，核心断言"日报与洞察的时间窗必须重叠"，串行实现永不通过）+ `pytest tests/test_closed_loop_stream.py`（16 例回归）+ 真机两次计时。
- **经验**：性能类问题必须先**逐段实测**再定位，否则会像本方案原稿那样把根因归给"LLM 慢"而修错地方（并行化虽有收益，但真正的大头是自家端点的空等）。
...
# 七、实施期新增发现（由 A1 真机场景冒烟抓出，本方案原稿未预见）

> 这三条都是"jsdom 结构性测不出、只有真实浏览器+真实运行才能发现"的问题，恰好印证了本轮 A1/A5 的立项理由。

**A6｜闭环配置 Modal 高于视口，底部按钮落在视口外 【已验证修复 · 高】**
- 发现：A1 场景运行器实测 `.ant-modal` 固定 680×1099，`.ant-modal-body` 无 `max-height`/`overflow`；
  1366×768 时面板可见比例仅 **0.524**、1280×720 降到 **0.48**（按 A1 的三口径即 FAIL），
  「确定并开始」按钮位于 **y=1123 —— 完全在视口外**，必须滚动 `.ant-modal-wrap` 才点得到。
- 修复：`RunConfigModal` 的 Modal 增加 `centered` + `styles.body = { maxHeight: calc(100vh - 240px), overflowY: auto }`，
  保证表头/底部按钮固定、内容区内部滚动。
- 验证：`browser_smoke --only closed-loop-config` 在 **1280×720 与 1366×768 双尺寸 PASS**；
  `browser_smoke --window 1280x720` 全量 **6/6 PASS、0 WARN**。

**B11｜僵尸运行把闭环互斥锁死（缺崩溃恢复） 【已验证修复 · 高】**
- 发现：A1 冒烟运行期间我执行了后端 rebuild → 进行中的 run 随进程消失，`run_records` 残留 `status=running`；
  由于 `/closed-loop/stream` 用 `get_running_run()` 做互斥，**此后所有闭环运行一律 409**，
  且 `cancel` 只写协作式标志，对"已失去执行者"的运行无效，只能等 30 分钟超时回收。
- 修复：新增 `run_service.reap_orphaned_runs()`（把所有 running 结算为 error，注明"后端进程重启导致运行中断"），
  在 `main.py` 的 lifespan 启动阶段调用（单实例部署前提：启动瞬间不可能有本实例正在跑的运行；多副本时需改为实例标识/心跳判定）。
- 验证：单测 3 例（含"刚创建的 running 也必须收割"与"不得误伤已结算运行"）；
  真机 `6984341a3b6f` 的 `error` 字段实测变为"后端进程重启导致运行中断（启动时崩溃恢复）"；
  随后闭环运行恢复正常，`closed-loop-run-timeline` 场景由 WARN 转为 **PASS**。

**B12｜`/closed-loop/stream` 同时承担"创建运行"与"订阅流" 【记录待评估 · 中】**
- 现象：任何重连/重试都会因互斥返回 409，用户无法自助恢复（只能在界面上看到"已有进行中的运行（run_id=…）"）。
- 本轮不改（属契约层设计），记入后续评估：可选方向是"允许携带 run_id 重连订阅既有运行"或提供"强制接管/取消并重跑"。
- 另：`cancel` 对僵尸运行无效的问题已由 B11 的启动收割覆盖主要场景，但"运行中进程假死"仍只能靠 30 分钟回收。
